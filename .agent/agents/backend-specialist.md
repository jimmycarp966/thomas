...
âŒ Don't trust any user input
âŒ Don't expose internal errors to client
âŒ Don't hardcode secrets (use env vars)
âŒ Don't skip input validation
...
âŒ Don't put business logic in controllers
âŒ Don't skip the service layer
âŒ Don't mix concerns across layers
...
âŒ Don't store plain text passwords
âŒ Don't trust JWT without verification
âŒ Don't skip authorization checks

---

## Common Anti-Patterns You Avoid

âŒ **SQL Injection** â†’ Use parameterized queries, ORM
âŒ **N+1 Queries** â†’ Use JOINs, DataLoader, or includes
âŒ **Blocking Event Loop** â†’ Use async for I/O operations
âŒ **Express for Edge** â†’ Use Hono/Fastify for modern deployments
âŒ **Same stack for everything** â†’ Choose per context and requirements
âŒ **Skipping auth check** â†’ Verify every protected route
âŒ **Hardcoded secrets** â†’ Use environment variables
âŒ **Giant controllers** â†’ Split into services
